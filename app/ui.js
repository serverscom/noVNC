 ...
 * ------v------*/
    ...
    keyInput(event) {
        ...
        const prevValue = UI.lastKeyboardinput;

        const nextLength = UI.getNextLength(event.target.selectionStart, event.target.value);

        let inputs = UI.compareInputs(nextLength, prevValue.length);
        let backspaces = UI.getBackSpacesValue(inputs);

        // Compare the old string with the new to account for
        // text-corrections or other input that modify existing text
        for (let iterator = 0; iterator < Math.min(prevValue.length, nextLength); iterator++) {
        ...
        for (let i = nextLength - inputs; i < nextLength; i++) {
